...
def main() -> None:
    ...
    status1, status2 = 0, get_api_answer(timestamp).get('homeworks')[0].get('status')
    ...
    while True:
        try:
            ...
            if get_api_answer(timestamp).get('homeworks')[0].get('status') not in HOMEWORK_VERDICTS:
                logger.error('неожиданный статус домашней работы')
                send_message(bot, 'неожиданный статус домашней работы')

            if status1 != status2:
                send_message(bot, parse_status(get_api_answer(timestamp).get('homeworks')[0]))
                logger.debug('cообщение в Telegram отправлено')
            else:
                logger.debug('отсутствие в ответе новых статусов')
            time.sleep(RETRY_PERIOD)
            status1, status2 = status2, get_api_answer(timestamp).get('homeworks')[0].get('id')
